...
different envelope? It did not. The diff against the debugpy adapter is the
measurement (`docs/DECISIONS.md` ADR-0001).

## What differs from debugpy, and where it went

| Delve fact | Where it landed |
| --- | --- |
| `dlv dap` listens on TCP; the client connects | `src/dap.ts` gained `connect()`; the host is unchanged |
